...
BEGIN
    ...
    IF TG_OP = 'DELETE' THEN
        ...
        VALUES (NULL, company_id_value, TG_TABLE_NAME, OLD.id, 'delete', to_jsonb(OLD));
        RETURN OLD;
    ELSIF TG_OP = 'UPDATE' THEN
        INSERT INTO audit_logs (user_id, company_id, table_name, record_id, action, old_values, new_values)
        VALUES (NULL, company_id_value, TG_TABLE_NAME, NEW.id, 'update', to_jsonb(OLD), to_jsonb(NEW));
        RETURN NEW;
    ELSIF TG_OP = 'INSERT' THEN
        INSERT INTO audit_logs (user_id, company_id, table_name, record_id, action, new_values)
        VALUES (NULL, company_id_value, TG_TABLE_NAME, NEW.id, 'create', to_jsonb(NEW));
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
